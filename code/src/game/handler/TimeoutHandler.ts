class TimeoutHandler {
	...
	public removePauseFn(fn: Function, thisObj) {
		for (var i = this.mPauseFns, a = i.length - 1; a >= 0; a--) i[a].fn == fn && i[a].fnThis == thisObj && i.splice(a, 1);
	}
	...
	public setAutoTimeout(fn, thisObj, time) {
		...
		for (var n = this, s = [], r = 3; r < arguments.length; r++) s[r - 3] = arguments[r];
		var o = -1;
	...
	public clearAutoTimeout(timeId) {
		...
		for (var t = this.autoTimeoutList, i = 0; i < t.length; i++) if (t[i].timeId == timeId) {
			t.splice(i, 1);
			break
		}
	}
	...
	public execAutoTimeout(timeId) {
		if (this.autoTimeoutList) for (var t = this.autoTimeoutList, i = void 0, a = 0; a < t.length; a++) if (i = t[a], i.timeId == timeId) {
	...
	public execAllAutoTimeout() {
		if (this.autoTimeoutList) {
			for (var e = this.autoTimeoutList, t = void 0, i = 0; i < e.length; i++) t = e[i], egret.clearTimeout(t.timeId), t.listener.call(t.thisObj, t.param);
			e.length = 0
		}
	}
