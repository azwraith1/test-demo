...
class PMDComponent extends game.BaseUI {
    ...
    private paomadeng(e: egret.Event) {
        ...
        pmd.key = data.gameId;
        if (data.type == 1) {
            pmd.text = '<font color="#FEDD00" size="22" fontFamily="Microsoft YaHei">' + data.content + '</font>';
            pmd.time = 3;
            pmd.time2 = 15000;
        } else if (data.type == 2) {
            let template = data.template;
            let content = data.content;
            let sceneName = '<font color="#FEDD00" size="22" fontFamily="Microsoft YaHei">' + content.sceneName + '</font>'
            let nameFont = '<font color="#FEDD00" size="22" fontFamily="Microsoft YaHei">' + content.nickname + '</font>'
            let gold = '<font color="#FEDD00" size="22" fontFamily="Microsoft YaHei">' + content.gold + '</font>'
