module game {
	export class RoomProxy extends ResourceProxyBase {
		...
		public async reconnectRoom() {
			...
			let config = Global.roomProxy.lastGameConfig;
		...
		public setRoomInfo(enterSceneResp: EnterSceneResp) {
			...
			if (this.roomInfo == null) {
				...
				if (this.roomInfo.serverTime) {
					game.DateTimeManager.instance.updateServerTime(this.roomInfo.serverTime);
				}
			}
		}
		...
		public getMineInfo(): PlayerGameDataBean {
			return this.roomInfo.players[this.playerInfo.playerIndex];
		}
